...
export function StatusPanel({ progress, isConnected, tasks }: StatusPanelProps) {
  ...
  return (
    <div className="bg-white border-2 border-gray-900 rounded-2xl shadow-3d overflow-hidden">
      {/* Header */}
      <div className="px-4 py-2.5 border-b-2 border-gray-900 flex items-center justify-between">
          ...
          <div>
            {inProgressTask && (
              <div className="mb-2.5">
                <span className="text-[11px] text-gray-400 uppercase tracking-wide">Working on</span>
                <p className="font-hand text-lg text-gray-900 font-medium leading-tight mt-0.5">
                  {inProgressTask.title}
                </p>
              </div>
            )}
            <div className="bg-gray-50 rounded-lg p-3 max-h-24 overflow-y-auto custom-scrollbar border border-gray-100">
              <p className="text-xs text-gray-600 whitespace-pre-wrap break-words leading-relaxed font-mono">
                {progress.message}
              </p>
            </div>
          </div>
