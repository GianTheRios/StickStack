...
// Store database in .stickstack directory in current working directory
const STICKSTACK_DIR = join(process.cwd(), '.stickstack');
const DB_PATH = join(STICKSTACK_DIR, 'kanban.db');
let db: Database | null = null;

export async function initDatabase(): Promise<void> {
  const SQL = await initSqlJs();

  // Ensure .stickstack directory exists
  if (!existsSync(STICKSTACK_DIR)) {
    mkdirSync(STICKSTACK_DIR, { recursive: true });
  }
